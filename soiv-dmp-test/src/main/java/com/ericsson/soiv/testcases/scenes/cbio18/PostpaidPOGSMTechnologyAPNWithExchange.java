...
public class PostpaidPOGSMTechnologyAPNWithExchange extends SoivTestBase
{
    ...
    @Description ("Logical Resource Exchange GSM APN Resources:BUC22B")
    public void postpaidPOGSMTechnologyAPNWithExchange() throws IOException
    {
        ...
        /*==========Exchange Resource NUmber==========*/
        /***************************
         * Fetching Extra resource GSM-APN
         ***********************************/
        ...
        resultInfo = Steps.PROTOCOLS.businessLogicBSCS
                .selectLogicalResource("Protocol : Get Free GSM-APN Number", this, customerInfo, ts, resultInfo)
                .run();
        log("Selected APN Exchange Resource: " + customerInfo.getresource().toString());
        customerInfo.setAPN(customerInfo.getresource().toString());
        ...
        ts.setResponseBody(jsonObject.toString());
        Steps.PROTOCOLS.businessLogicEOC.updatePooi("Provision : Update the Optional with Exchange Resource Number", this,
                customerInfo, ts, resultInfo).run();
        Steps.PROTOCOLS.businessLogicEOC
        ...
        body = RawMessages.getBodyFromTextFilePlacedInTestStep(this.getClass().getName(),
                "protocols/bscs/udrTemplates", ts.getUdrTextFileName()).toString();
        body = body.replace("{{IMSI}}", customerInfo.getLinkedPortNumber());
        body = body.replace("{{APN}}", customerInfo.getAPN());
        body = body.replace("{{DATE}}", calculateDateTime.getCurrentDateTime());
        ...
        Steps.CHARGING_OFFLINE.stepsChargingOfflineBscs
                .offlineGy("Protocol : BSCS - Offline Usage for Exchange Resource Number", this, resultInfo, customerInfo, ts)
                .run();
