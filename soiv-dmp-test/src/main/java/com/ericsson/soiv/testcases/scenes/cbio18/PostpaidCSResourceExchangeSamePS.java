...
public class PostpaidCSResourceExchangeSamePS extends SoivTestBase {
    ...
    public void postpaidCsResourceExchangeSamePs() throws IOException, InterruptedException {
        ...
        resultInfo = Steps.PROVISION.eoc
                .createShoppingCart("Provision : Create Shopping Cart For Optional IP PO", this, customerInfo, ts, resultInfo)
                .run();
...
// Updating the Optional PO with the resources IPV4
        ...
        Steps.PROTOCOLS.businessLogicEOC
                .updatePooi("Provision : Update the Optional  PO for resources IPV4", this, customerInfo, ts, resultInfo)
                .run();

        Steps.PROTOCOLS.businessLogicEOC.submitShoppingCart("Submit cart for Resources in Optional PO", this, customerInfo, ts, resultInfo)
                .run();
...
//Steps for Optional PO Exchange
        ts.setJsonFileName("createShoppingCart.json");
        Steps.PROVISION.eoc.createShoppingCart("Provision : Create Shopping Cart For Exchange Of PO",
        ...
        Steps.PROTOCOLS.businessLogicEOC
                .swapProductOffering("Protocol : Exchange Optional Product Offering", this, customerInfo, ts, resultInfo)
                .run();
...
// Optional_APN PO After Swap Should Be Active In the Contract
    ...
    private void verifyInvoicePDF(TransactionSpecification ts,String message) throws InterruptedException {
        ...
        if ("AFTER".equals(message)) {
                ...
                Jive.log("PO Name on Bill " + businessCache.get("PO.ID_OPTIONAL") + " Not Validated Successfully");
            }
