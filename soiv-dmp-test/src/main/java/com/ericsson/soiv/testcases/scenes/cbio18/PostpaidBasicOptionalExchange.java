...
public class PostpaidBasicOptionalExchange extends SoivTestBase {
    ...
    @Description("PostpaidBasicOptionalExchange UC01 BUC8BB")
    public void postpaidBasicOptionalExchange() throws IOException, InterruptedException {
        ...
        ts.setEventCode(businessCache.get("EVENT.CODE"));
        ts.setPrice(businessCache.get("ADMIN.CHARGES"));
        Steps.PROTOCOLS.businessLogicEOC.createCustomerChangeOrderAdminCharge(
                "Protocol : Apply For Admin Charges on Exchange", this, customerInfo, ts, resultInfo).run();
        ...
        verifyOptionalPOStatus(productOfferings.get("optional").toString());
    ...
    private void verifyOptionalPOStatus(String poName) {
